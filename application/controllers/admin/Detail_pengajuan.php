...
class Detail_pengajuan extends CI_Controller {
	...
	function index($pengajuan_id) {
		$data['nav'] = "admin/detail_pengajuan/nav";
		$data['isi'] = "admin/detail_pengajuan/index";
	...
	function hapus($id, $standar) {
		...
		redirect(base_url('admin/detail_pengajuan/index/' . $berkas->pengajuan_id . '?tab=' . $standar));
	}
	...
	function upload() {
		...
		redirect(base_url('admin/detail_pengajuan/index/' . $pengajuan->id . "?tab=" . $this->input->post('last_tab')));
	}
