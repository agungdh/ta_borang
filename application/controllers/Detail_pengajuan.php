...
class Detail_pengajuan extends CI_Controller {
	...
	function index($pengajuan_id) {
		$data['nav'] = "detail_pengajuan/nav";
		$data['isi'] = "detail_pengajuan/index";
	...
	function hapus($id, $standar) {
		...
		redirect(base_url('detail_pengajuan/index/' . $berkas->pengajuan_id . '?tab=' . $standar));
	}
	...
	function upload() {
		...
		redirect(base_url('detail_pengajuan/index/' . $pengajuan->id . "?tab=" . $this->input->post('last_tab')));
	}
	...
	function unduh_semua($pengajuan_id) {
		...
		if ($persentase > 0) {
			...
			$flashdata['pesan'] = "Gagal Download !!!";
